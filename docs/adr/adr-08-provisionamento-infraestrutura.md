...
Requer credenciais GCP configuradas no GitHub Secrets. **Decisão**: ✓ Resolvido — a autenticação usa **Workload Identity Federation (WIF)**, que autentica o GitHub Actions no GCP via tokens OIDC de curta duração, sem chave JSON persistida como secret. O mecanismo original planejado — chave JSON de service account armazenada como GitHub Secret (`GOOGLE_CREDENTIALS`) — foi descartado durante a implementação: a org policy `constraints/iam.disableServiceAccountKeyCreation` estava ativa no projeto GCP, bloqueando a criação de chaves JSON para qualquer service account. A política não pôde ser desabilitada por ausência de permissão de admin de organização. A decisão resultante foi implementar WIF, que além de ser a abordagem bloqueante-safe é também a recomendação atual do Google para autenticação de GitHub Actions no GCP: um WIF pool (`github-pool`), um provider OIDC (`github-provider`) e um binding de `principalSet` vinculando a service account `sa-terraform-ci` especificamente ao repositório `lucasricardoaa/anac-flight-pipeline`. Os secrets configurados no GitHub são `WIF_PROVIDER` (nome canônico do provider) e `WIF_SERVICE_ACCOUNT` (email da service account). A service account `sa-terraform-ci` recebe `roles/editor` no projeto e `roles/storage.objectAdmin` no bucket de state.
