...
| 8 | ADR-06 | ✓ Resolvido: e2-medium em `us-central1` — ~$0.0336/hora (~$24/mês). Dentro da estimativa de $25–30/mês. Com $300 de free trial, a VM suporta ~12 meses de operação. |
...
| 15 | ADR-08 | ✓ Resolvido: Workload Identity Federation implementado — org policy `constraints/iam.disableServiceAccountKeyCreation` bloqueou criação de chaves JSON. WIF pool `github-pool` + provider OIDC `github-provider` + binding `principalSet` para o repositório. Secrets configurados: `WIF_PROVIDER` e `WIF_SERVICE_ACCOUNT`. |
