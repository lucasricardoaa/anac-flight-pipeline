...
| 10 | ADR-08 | Definir durante a implementação se o script de bootstrap do Docker na VM do Airflow é declarado como `metadata_startup_script` no recurso `google_compute_instance` do Terraform ou se é uma etapa de configuração separada documentada no README. A escolha afeta a reprodutibilidade: se o script não é parte do Terraform, o provisionamento via `terraform apply` produz uma VM sem Docker instalado, exigindo passo adicional manual ou documentado. |
| 11 | ADR-08 | Definir quais buckets GCS têm lifecycle rules configuradas e qual política de expiração é adequada para cada camada da arquitetura Medalhão. A camada Bronze, como fonte da verdade imutável (ADR-07), não deve ter lifecycle rules de expiração sobre dados ingeridos. Objetos temporários gerados por jobs Spark são candidatos a expiração — o escopo exato precisa ser determinado durante a implementação. |
| 12 | ADR-08 | Mapear durante a implementação o conjunto exato de roles IAM necessárias para cada service account do projeto (uma para o Airflow, uma para o Dataproc). Roles excessivamente permissivas (`roles/owner`, `roles/editor`) não são adequadas para componentes com escopos funcionais delimitados. A lista de roles deve seguir o princípio de menor privilégio com base nas operações reais que cada componente executa. |
...
| 17 | ADR-08 | Confirmar que `terraform.tfstate` e `terraform.tfstate.backup` estão listados no `.gitignore` do repositório. O arquivo de state pode conter valores sensíveis de configuração de recursos GCP e nunca deve ser commitado. Este ponto se aplica mesmo que remote state em GCS seja adotado — o arquivo local gerado durante o bootstrap precisa estar excluído. |
